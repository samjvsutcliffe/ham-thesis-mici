...
def get_data_all(folder,frame_number):
    ...
    regex = re.compile(r'sim_real_{}_{}.vtk'.format(frame_step,frame_number))
...
chalk_dir ="/nobackup/rmvn14/thesis/mici"
...
offset = h*2
water_height = water_height - offset
...
finalcsv = re.compile("sim_real_\d+_\d*\.vtk")
files_csvs = list(filter(finalcsv.match,files))


#step_list = list(filter(output_regex.match,files_csvs))
step_list = []
for f in files_csvs:
    s = f.split("_")[-2]
    if not s in step_list:
        step_list.append(s)
step_list.sort(key=int)
for i,out in enumerate(step_list):                                    
    print("{}: {}".format(i,out))                                       
frame_step = step_list[int(input())]

finalcsv = re.compile("sim_real_{}_\d*\.vtk".format(frame_step))
files_csvs = list(filter(finalcsv.match,files))
...
def get_plot(i,fname):
    ...
    ax.set_xlim(xlim)
    ax.set_ylim(ylim)
    plt.title("Frame {} - T {:.2f}s - {} - E: {} - OOBF: {:.2E}".format(i,
